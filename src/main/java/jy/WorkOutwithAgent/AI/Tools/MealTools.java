...
public class MealTools {
    ...
    public List<MealResponseDto> findMealsByUsernameForAI(
        ...
        List<Meal> meals = mealRepository.findByMember_UsernameAndMealDateBetween(username, startOfDay, endOfDay);
        return meals.stream()
                .map(MealResponseDto::fromEntity)
                .collect(Collectors.toList());
    }

    @Tool("사용자 식별자로 회원의 최근 식사정보를 조회합니다.  memberId는 회원의 고유식별자(id), limit은 최근 N개의 식사정보를 가져올건지의 개수. 개수 요청이 없으면 1개만 반환하세요.")
